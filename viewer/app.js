...
document.getElementById("highlight-btn").addEventListener("click", async () => {
  ...
  try {
    const res = await fetch(
      `/api/pair-ids?country=${encodeURIComponent(country)}&resource=${encodeURIComponent(resource)}`
    );
...
for (const btn of insightBtns) {
  btn.addEventListener("click", async () => {
    ...
    insightStatus.textContent = "Computing…";

    try {
      const res = await fetch(`/api/analysis/${analysis}`);
